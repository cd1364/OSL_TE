...
previous command is successfully executed or not)
read name
test $name = Ramesh
echo $?
...
grep -i 'Chaitanya' ./myaddressbook.txt
# 2) Return the count/ occurance of given string
grep -c '15' ./myaddressbook.txt
# 3) Return the lines contains given string also adds line number
at begining
grep -n '15' ./myaddressbook.txt

## sed
# 1) Replace 15 with 30
echo "Before Replacing Number"
cat ./myaddressbook.txt
echo "\nAfter Replacing Number"
sed 's/15/30/' ./myaddressbook.txt